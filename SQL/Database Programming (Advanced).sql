...
CREATE TRIGGER trg_hitung_total_gaji
BEFORE INSERT ON gaji
FOR EACH ROW
BEGIN
    SET NEW.total_gaji = 
        IFNULL(NEW.gaji_pokok,0) 
        + IFNULL(NEW.tunjangan,0) 
        - IFNULL(NEW.potongan,0);
END$$
...
BEGIN
    ...
    SELECT total INTO total_penjualan
    FROM penjualan
    WHERE id_penjualan = NEW.id_penjualan;

    SET NEW.jumlah_bayar = total_penjualan;
END$$
